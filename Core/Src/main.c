...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim->Instance == TIM2)   // Timer 1ms
	{
		...
		case BTN_WAIT_NEXT_PRESS:
			// Kiểm tra lần nhấn tiếp theo (EXTI event)
			if (exti_flag && btn_pin_state == GPIO_PIN_SET)
			{
				// Có lần nhấn tiếp theo → bắt đầu debounce lần nhấn mới
				btn_state = BTN_DEBOUNCE;
				debounce_cnt = 0;
				exti_flag = 0;
			}
